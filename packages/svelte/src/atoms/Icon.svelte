<script lang="ts">
  import type { IconAtom } from '@daui/core';
  ...
  let { name, size = 'md', color, class: className = '' }: Omit<IconAtom, 'atom'> = $props();
...
{#if IconComponent}
  <IconComponent class="{sizeClass} {className}" style={color ? `color: ${color}` : undefined} />
{:else}
