<script lang="ts">
  import type { ChartAtom } from "../types";
  ...
  $: baseId = id ?? instanceId;
  $: gradientId = `${baseId}-area`;
  const curveMap = {
    linear: curveLinear,
    monotone: curveMonotoneX,
    natural: curveNatural
  } as const;
  $: chartCurve = curveMap[curve];
  $: motion = animate ? true : undefined;
  ...
  {:else}
    ...
    >
      ...
      <Tooltip.Root let:data>
        <Tooltip.Content class="bg-popover border rounded-lg shadow-lg px-3 py-2">
